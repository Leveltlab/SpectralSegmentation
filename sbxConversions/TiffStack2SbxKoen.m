...
for i = 1:nfiles
    ...
    if nframes > 65536 % check if bigTiff

        % Open the BigTIFF file
        t = Tiff(fileTif, 'r');
        frameCounter = 0; % Initialize a counter to track frames written

        hwb = waitbar(0, 'Writing SBX file...');
        for f = 1:nframes
            t.setDirectory(f); % Move to the correct frame
            im = uint16(t.read()) .* multiplier; % Read and process the frame
            fwrite(fileID, im, 'uint16'); % Write the frame
            frameCounter = frameCounter + 1; % Increment the counter
        ...
        fprintf('Frames written to SBX file: %d\n', frameCounter);
        fprintf('Successfully processed %d frames from BigTIFF file.\n', nframes);
