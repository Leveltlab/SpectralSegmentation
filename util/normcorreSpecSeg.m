function [M_final,shifts,template,options] = normcorreSpecSeg(Y,options,template)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    elseif strcmpi(ext,'sbx') % Added sbx compatibility
        filetype = 'sbx';
        sizY = [options.d1 options.d2 options.max_idx];
        T = options.max_idx;
        Sbx = memmapfile(Y, 'Format', 'uint16');
        
        Skipframe = inf; %bug in scanbox frame recording at 65538 frames
        if isfield(options, 'Skipframe')
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    case 'sbx' %Img = sbxread(strfp, (Pos-1)*Splitnm + Slice,1);
            nsamp = options.nsamples/2; %all samples for one frame * number of channels
            if isfield(options, 'bSplit') && options.bSplit == 1
                Slices = options.Slices; %number of (depth) slices in recording
                Slice = options.Slice; %(zero based, 0, 1, 2, ..)
                samples = (Slice*nsamp+1:Slice*nsamp+init_batch*nsamp*Slices);
                Y_temp = Sbx.Data(samples); 
                Y_temp = reshape(Y_temp,[options.Shape init_batch*Slices]);
                Y_temp = Y_temp(:,:,:,1:Slices:init_batch*Slices); %select one depth slice over time
            else
                samples = 1:init_batch*nsamp;
                Y_temp = Sbx.Data(samples);
                Y_temp = reshape(Y_temp,[options.Shape init_batch]);
            end
            ...
            if options.scanbox_version == 2.5 || simon
                if nd == 2
                     if options.nchan == 2
                         Y_temp = Y_temp(options.crop.x,options.crop.y,:,:); 
                         dim = size(Y_temp);
                         Y_temp = reshape(Y_temp, dim(1), dim(2)*2, []);
                     else          
                        Y_temp = squeeze(Y_temp(options.crop.x,options.crop.y,1,:)); 
                     end
                end  
            else
                if nd == 2 
                    if options.nchan == 2
                        Y_temp = Y_temp(:,options.crop.x,options.crop.y,:); 
                        Y_temp = permute(Y_temp, [2 3 1 4]);
                        dim = size(Y_temp);
                        Y_temp = reshape(Y_temp, dim(1), dim(2)*2, []);
                    else
                        Y_temp = squeeze(Y_temp(1,options.crop.x,options.crop.y,:)); 
                    end
                end  
            end
            %Y_temp = permute(Y_temp, options.Perm);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
    case 'dat'
        ...
        fid = fopen([options.h5_filename], 'w');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
    otherwise
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                
           case 'sbx'
                    nsamp = options.nsamples/2;
                    if isfield(options, 'bSplit') && options.bSplit == 1
                        Slices = options.Slices; %number of (depth) slices in recording
                        Slice = options.Slice; %(zero based, 0, 1, 2, ..)
                        frameindx = ((t-1)*Slices + Slice);
                        
                        %adapt for scanbox bug
                        frameindx = frameindx + floor(frameindx/Skipframe);

                        Y_temp = Sbx.Data(frameindx*nsamp+1:(frameindx + 1)*nsamp);
                    else
                        frameindx = t-1;
                        %adapt for scanbox bug
                        frameindx = frameindx + floor(frameindx/Skipframe);
                    ...
                    Y_temp = reshape(Y_temp, options.Shape);
                    if ~simon
                        Y_temp = intmax('uint16')- Y_temp;
                    end
                    if options.scanbox_version == 2.5 || simon
                        if nd == 2 
                            if options.nchan == 2
                                ...
                                Yt = single(Y_temp(options.crop.x,options.crop.y,1)); 
                            end
                        end 
                        %Yt = permute(Yt, [2 1 3 4]);
                    else
                        if nd == 2
                            if options.nchan == 2
                                Y_temp = Y_temp(:,options.crop.x,options.crop.y); 
                                ...
                                Yt = single(squeeze(Y_temp(1,options.crop.x,options.crop.y))); 
                            end
                        end
                        
%                         if nd == 3 
%                             Yt = single(Yt(:,options.crop.y,options.crop.x,:)); 
%                             Yt = permute(Yt, [2 3 1 4]);
%                         end 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                
            case 'dat'
                if rem_mem == options.mem_batch_size || t== T
                    if nd == 2, fwrite(fid, mem_buffer(:,:,1:rem_mem), data_type); end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if strcmp(options.output_type, 'dat')
    fclose(fid);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
