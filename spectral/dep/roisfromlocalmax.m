...
%      notS = 0;     %num of non significant maxes
      Cnt = Savedrois.Cnt;
      for i = 1:Nmp
%if debug
%           if pnt(i,1) ==  278 && pnt(i,2) == 327
%               disp('pnt')
%           end
  ...
  %   plot(Con.x, Con.y, 'r')                         
                          ...
                          if Cnt > 0
                              ...
                              if ~isempty(In)
  %figure(3), imagesc(Mask(lxw:hxw, lyw:hyw) + F.*Cnt)
                         ...
                         if bval
                             ...
                             yrange = lyw:hyw;
                             xrange = lxw:hxw;                             
                             [NwCon, NwA, NwF, NwV] = PixelCor(size(Mask), F, sbxt, py, px, Iy, Ix, yrange, xrange, freq);
                             if ~isempty(NwCon) &&  NwA > area(1) %exists and area greater than minimum
                                 Con = NwCon;
                                 Con.y = Con.y + lyw-1;
                                 Con.x = Con.x + lxw-1;
                                 F = NwF;
...
%      plot(vx-wx(1)+1, vy-wy(1)+1, 'r')
                 ...
                 [NwCon, NwA, NwF, NwV] = PixelCor(size(Mask), Mz, sbxt, py, px, iy, ix, wy, wx, freq);
                 if ~isempty(NwCon) &&  NwA > area(1) %exists and area greater than minimum
                    Mz(NwF==1) = 0;
                    %first original pnt
