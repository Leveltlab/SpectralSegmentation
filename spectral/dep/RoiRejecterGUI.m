...
% Last Modified by GUIDE v2.5 19-Feb-2019 14:57:02
...
function backGrdView_CreateFcn(hObject, ~, ~) %#ok<DEFNU>
    ...
    hObject.String = {'Spectral', 'ROI corr', 'Mask', 'Chronic (red) & spectral (green)',...
...
%     currentView = [h.mainAx.XLim, h.mainAx.YLim];
    ...
    if selected ~= switches.viewToggle
        ...
        switch selected
            ...
            case 2 % ROI corners signal correlations image
            ...
            case 4 % Chronic & current backgroudn overlay view
            ...
            case 6 % Colors are based on what the peak frequency is
                picMask  = data.Mask;
            ...
            case 7 % Import variable from workspace
...
function saveButton_Callback(~, ~, h) %#ok
    ...
    if data.PP.Cnt == switches.originalCnt
        ...
        pause(2)
        h.saveButton.String = {'save to workspace'};
    else
        h.SaveButton.String = {'to workspace'};
        h.saveButton = TurnOn(h.saveButton);
        
        assignin('base', 'Mask', data.Mask);
        assignin('base', 'PP', data.PP);
        assignin('base', 'BImg', data.BImg);
