...
function data = main()
    ...
    %optimise based on mask

    noChange = affinetform2d();
    binOptimise = optimiseFolder(data.binaryMask{1}, data.binaryMask(2:end));

    plotImagestFormBulk(data.binaryMask, [noChange, binOptimise.netTForm(:)'], fileInfo.filenamesShort);
    plotImagestFormBulk(data.cleanImg, [noChange, binOptimise.netTForm(:)'], fileInfo.filenamesShort);
    %optimise based on real img
    bImgOptimise = optimiseFolder(data.cleanImg{1}, data.cleanImg(2:end));
...
function outputTable = optimiseFolder(fixed, Images, nFiles)
    arguments (Input)
        fixed (:, :) double
        Images (:, :) cell
        nFiles double = length(Images)
    ...
    columnNames =  {'moving',  'netTForm'};
    varTypes = {'cell','affinetform2d'};
    rowNames =  arrayfun(@(i) sprintf('transform from image: (%d -> fixed)', i), 1:nFiles, 'UniformOutput', false);
    outputTable = table('Size', [nFiles, length(columnNames)], 'VariableTypes',varTypes, 'VariableNames',columnNames, 'RowNames',rowNames);
